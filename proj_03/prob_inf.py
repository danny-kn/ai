...
def calc_global_joint_prob(model, variableValues):
  ...
  global_joint_probability = 1
  for index in range(0, len(model.vars)):
    if variableValues[model.vars[index]] == True:
      # The contract states that read_cpt() returns P( Query | Evidence = True ).
      conditional_probability = read_cpt(model, model.vars[index], variableValues)
    else:
      conditional_probability = 1 - read_cpt(model, model.vars[index], variableValues)
    global_joint_probability *= conditional_probability
  return global_joint_probability
...
def calc_query_exact_brute(model, queryVar, queryVal, evidence):
  ...
  for jptEntry in truefalse_combination_iterator(model.vars):
    ...
    # Calculating P(Query and Evidence):
    if jptEntry[queryVar] == queryVal and dict_issubset(jptEntry, evidence):
      pr_QE += pr_entry
...
def recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc):
  ...
  if marginalize:
    ...
    prQ_T_T, prQ_F_T = recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc[1:])
    variableValues[var] = False
    prQ_T_F, prQ_F_F = recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc[1:])
    prQ_T = prQ_T_T + prQ_T_F
    prQ_F = prQ_F_T + prQ_F_F
