...
def linear_regression(X, Y):
    ...
    Output:
        p_hat: the linear parameter vector. Please represent it as a D-by-1 matrix (numpy array).
    Useful tool(s):
    ...
    p_hat = np.matmul(np.matmul(np.linalg.inv(np.matmul(A.transpose(), A)), A.transpose()), Y)
    # Note: Referenced Slide 19 {Linear Model Parameter Estimation}.
    ### Your Job Ends Here ###
    return p_hat
...
def polynomial_regression(X, Y, degree):
    ...
    Output:
        p_hat: the linear parameter vector. Please represent it as a D-by-1 matrix (numpy array).
           p[0] should be the constant term, p[1] the linear term, p[2] the quadratic term, etc.
    Useful tool(s):
        1. ** operator.
    ...
    for index in range(0, num_columns):
        A[:, index] = X[:, 0] ** index
    # Note: Referenced Slide 22 {Linear Model Parameter Estimation}.
    ### Your Job Ends Here ###
    return linear_regression(A, Y)
